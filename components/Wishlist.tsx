...
const SmallJoy: React.FC<WishlistItemComponentProps> = ({ 
    item, isEditing, editForm, setEditForm, onEdit, onSave, onCancel, onDelete, onToggle 
}) => {
    if (isEditing) {
        return (
    ...
    return (
        <div className={`p-4 rounded-2xl transition-all border group/card relative ${item.completed ? 'bg-amber-50/50 border-amber-100 opacity-60' : 'bg-white/60 border-white hover:border-amber-200 hover:shadow-lg hover:-translate-y-1'}`}>
            <div className="absolute top-2 right-2 opacity-0 group-hover/card:opacity-100 transition-opacity z-10">
                 <button onClick={() => onEdit(item)} className="p-1.5 text-slate-400 hover:text-amber-500 bg-white/80 rounded-full shadow-sm hover:shadow">
                    <Pencil size={14} />
                 </button>
            </div>
...
const LongTermGoal: React.FC<WishlistItemComponentProps> = ({ 
    item, isEditing, editForm, setEditForm, onEdit, onSave, onCancel, onDelete, onToggle 
}) => {
    if (isEditing) {
        return (
             <div className="p-6 rounded-3xl bg-white border-2 border-purple-200 shadow-xl flex flex-col gap-4 animate-in fade-in zoom-in-95 duration-200">
                <div className="flex justify-between items-center mb-2">
                    ...
                    <button onClick={() => onDelete(item.id)} className="text-xs text-red-400 hover:text-red-500 hover:underline">Delete</button>
                </div>
                <textarea 
                ...
                <div className="flex justify-end gap-2">
                    <button onClick={onCancel} className="px-4 py-2 bg-slate-100 text-slate-600 rounded-xl hover:bg-slate-200 transition-colors font-medium">Cancel</button>
                    <button onClick={onSave} className="px-6 py-2 bg-purple-500 text-white rounded-xl hover:bg-purple-600 transition-colors flex items-center gap-2 shadow-lg shadow-purple-200 font-medium"><Save size={18}/> Save Vision</button>
                </div>
             </div>
        );
    }
    ...
    return (
        <div className={`relative overflow-hidden p-6 rounded-3xl transition-all border group ${item.completed ? 'bg-slate-50 border-slate-200' : 'bg-gradient-to-br from-indigo-50 to-purple-50 border-white hover:shadow-xl'}`}>
            ...
            <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                 <button onClick={() => onEdit(item)} className="p-2 text-slate-400 hover:text-purple-600 bg-white/60 hover:bg-white rounded-full shadow-sm backdrop-blur-sm transition-all">
                    <Pencil size={16} />
                 </button>
            </div>
